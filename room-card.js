...
class RoomCard extends HTMLElement {
  ...
  set hass(hass) { this._hass = hass; if (!this.content) this.render(); this.updateContent(); }
  setConfig(config) { this.config = config; if (!this.content) this.render(); this.updateContent(); }
  ...
  updateContent() {
    ...
    const ico = this.shadowRoot.getElementById("icon"); ico.icon = c.icon || "mdi:home"; ico.style.color = c.color || "white";
    ...
    this.shadowRoot.getElementById("info").innerText = `${(t!=null&&t!="-")?t+"°C":"–"} | ${(hm!=null&&hm!="-")?hm+"%":"–"}`;
  ...
  _nav() { if(this.config.tap_action?.action==="navigate"&&this.config.tap_action?.navigation_path) { history.pushState(null,"",this.config.tap_action.navigation_path); window.dispatchEvent(new Event("location-changed",{bubbles:true,composed:true})); } }
...
class RoomCardEditor extends HTMLElement {
  constructor() { super(); this.attachShadow({ mode: "open" }); }
  setConfig(config) { this._config = config || {}; if(!Array.isArray(this._config.controls)) this._config = {...this._config, controls:[]}; this.render(); }
  set hass(hass) { 
  ...
  render() {
    if(!this._config) return; 
    if(this.shadowRoot.innerHTML) { this.updVal(); this.renBtn(); return; }
    const h = this._hass;
    this.shadowRoot.innerHTML = `
      <style>
      ...
      <div class="sec"><h3>${getTranslation(h,"sensors_manual")}</h3>
      ...
      <div class="sec"><h3>${getTranslation(h,"buttons")}</h3><div id="b"></div><mwc-button id="add" raised label="${getTranslation(h,"add_button")}"><ha-icon icon="mdi:plus" slot="icon"></ha-icon></mwc-button></div>`;
